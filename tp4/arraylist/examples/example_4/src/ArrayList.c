...
int al_push(ArrayList* pList, int index, void* pElement)
{
    ...
    if(pList != NULL && pElement != NULL && (index >= 0 && index <= pList->size) && !resizeUp(pList))
    {
        expand(pList,index);
        returnAux = 0;
        pList->size++;
...
int al_containsAll(ArrayList* pList,ArrayList* pList2)
{
    ...
    if(pList != NULL && pList2 != NULL)
    {
            ...
            for(i=0;i<pList->size;i++)
            {
                if((pList->pElements[i] != pList2->pElements[i]) || (pList->size != pList2->size))
                {
                    returnAux = 0;
                    break;
                }
...
int al_sort(ArrayList* pList, int (*pFunc)(void* ,void*), int order)
{
    ...
    if(pList != NULL && pFunc != NULL && (order == 0 || order == 1))
    {
        for(i=0;i-1<pList->size;i++)
        {
            ...
            {
                if((pFunc(pList->pElements[i],pList->pElements[j])==1 && order== 1) || (pFunc(pList->pElements[i],pList->pElements[j])==-1 && order== 0))
                {
